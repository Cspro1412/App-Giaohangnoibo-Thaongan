...
let vehicles = ["Xe Tải 1", "Xe Tải 2"];
...
function getTruckIcon(truckName) {
    ...
    if (idx === -1) idx = 0; 
    let color = routeColors[idx % routeColors.length]; 
    
    let html = `
        <div style="background-color: ${color}; width: 34px; height: 34px; border-radius: 50%; border: 3px solid white; box-shadow: 0 3px 6px rgba(0,0,0,0.4); display: flex; align-items: center; justify-content: center; font-size: 16px; color: white;">
            🚚
        </div>
    `;
...
async function drawFixedRoute() {
    ...
    for (let i = 0; i < routesToProcess.length; i++) {
        ...
        let activeCustomers = customers.filter(c => c.selected && c.ten_xe === vName && c.lat !== null && c.lng !== null);
    ...
    ws.onmessage = function(event) {
        ...
        if (msg.type === "REFRESH_LIST") {
            ...
            if(!liveTruckMarkers[t.ten_xe]) {
                liveTruckMarkers[t.ten_xe] = L.marker([t.lat, t.lng], {icon: getTruckIcon(t.ten_xe)})
                    .bindTooltip("🚚 " + t.ten_xe, {permanent: true, direction: 'top', offset: [0, -20], className: 'truck-tooltip'})
                    .addTo(map);
            } else {
                liveTruckMarkers[t.ten_xe].setLatLng([t.lat, t.lng]);
            }
        }
    };
...
async function loginAdmin() {
    ...
    try {
        let res = await fetch(`${API_URL}/api/xac-thuc`, {
            method: "POST", headers: {"Content-Type": "application/json"},
            body: JSON.stringify({ mat_khau: pwd })
        });
        ...
        alert("⚠️ Không kết nối được máy chủ Python!");
        btn.innerText = oldText;
    }
}
...
L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: '&copy; OpenStreetMap' }).addTo(map);
    ...
    if (savedLocal) {
        ...
        if (parsed.vehicles && parsed.vehicles.length > 0) vehicles = parsed.vehicles;
    }
    ...
    try {
        ...
        if (result.thanh_cong) {
            ...
            for (let truckName in liveTruckMarkers) {
                if (!activeTrucks.includes(truckName)) {
                    map.removeLayer(liveTruckMarkers[truckName]);
                    delete liveTruckMarkers[truckName];
                }
            }
...
function updateMapFont() {
    ...
    styleTag.innerHTML = `.leaflet-tooltip.custom-tooltip, .leaflet-tooltip.warehouse-tooltip, .leaflet-tooltip.master-tooltip { font-family: ${fontFamily} !important; font-size: ${fontSize} !important; } .leaflet-tooltip.master-tooltip { color: ${fontColor} !important; }`;
    localStorage.setItem("GiaoHangFontSettings", JSON.stringify({ fontFamily, fontSize, fontColor }));
}
...
function toggleMasterDataMap() {
    ...
    if (isShowingMasterData) {
        btn.innerHTML = "👀 ẨN TOÀN BỘ DANH BẠ KHỎI BẢN ĐỒ";
        btn.style.backgroundColor = "#0ea5e9";
    } else {
        btn.innerHTML = "👀 HIỆN TOÀN BỘ DANH BẠ LÊN BẢN ĐỒ";
...
function renderMasterDataOnMap() {
    ...
    validData.forEach(d => {
        L.marker([d.lat, d.lng], {icon: masterIcon}).bindTooltip(d.ten_khach, { permanent: showLabels, direction: d.dir, className: "master-tooltip" }).addTo(masterDataMarkersLayer);
    });
}

function saveLocalConfig() { localStorage.setItem('GiaoHangConfig', JSON.stringify({ warehouses: warehouses, vehicles: vehicles })); }

async function fetchCustomersFromAPI() {
    if (document.querySelector('.dragging')) {
        setTimeout(fetchCustomersFromAPI, 2000);
        return;
    }
    ...
    try {
        ...
        if (result.thanh_cong) {
            ...
            customers = result.data.map(c => ({
                ...
                rawInput: c.rawInput, ten_xe: c.ten_xe, trang_thai: c.trang_thai, thu_tu: c.thu_tu, ghi_chu: c.ghi_chu,
...
function addNewVehicle() {
    ...
    if (!input || vehicles.includes(input)) return;
    vehicles.push(input); document.getElementById("new-vehicle-name").value = ""; saveLocalConfig(); updateVehicleUI();
}

function deleteVehicle(vName) {
    if(vehicles.length <= 1) { alert("Phải giữ lại ít nhất 1 xe!"); return; }
    if (confirm(`Xóa ${vName}?`)) {
        vehicles = vehicles.filter(v => v !== vName);
        if(currentVehicleFilter === vName) currentVehicleFilter = "all";
        saveLocalConfig(); updateVehicleUI();
    }
}

function updateVehicleUI() {
    let tagsDiv = document.getElementById("vehicle-tags");
    tagsDiv.innerHTML = vehicles.map(v => `<span class="route-chip">${v} <span class="route-delete" onclick="deleteVehicle('${v}')" title="Xóa">×</span></span>`).join('');
    let options = vehicles.map(v => `<option value="${v}">${v}</option>`).join('');
    document.getElementById("quick-vehicle-select").innerHTML = options;
    document.getElementById("bulk-vehicle-select").innerHTML = options;
    document.getElementById("single-vehicle-select").innerHTML = options;
    let filterSelect = document.getElementById("filter-vehicle-select");
    filterSelect.innerHTML = `<option value="all">🌍 HIỂN THỊ TẤT CẢ CÁC XE</option>` + vehicles.map(v => `<option value="${v}">🚛 Xe: ${v}</option>`).join('');
    filterSelect.value = currentVehicleFilter;
}

function onFilterChange() { currentVehicleFilter = document.getElementById("filter-vehicle-select").value; clearRoutes(); renderCustomerList(); }
...
async function addCustomerFromQuickSearch(ma, ten, dulieu) {
    ...
    try {
        let res = await fetch(`${API_URL}/api/them-khach-hang`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ ma_khach: ma, ten_khach_hang: ten, du_lieu_goc: dulieu, ten_xe: xe })
        });
        let result = await res.json();
        if (result.thanh_cong) { if(!dulieu) alert(`⚠️ Đã thêm [${ten}] vào [${xe}] nhưng khách này đang thiếu tọa độ!`); } 
...
async function addSingleCustomer() {
    ...
    try {
        let res = await fetch(`${API_URL}/api/them-khach-hang`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ ma_khach: name, ten_khach_hang: name, du_lieu_goc: input, ten_xe: xe })
        });
        let result = await res.json();
        if (result.thanh_cong) {
            document.getElementById("single-name").value = ""; document.getElementById("single-coords").value = "";
            if(!result.has_coords && !input) alert(`⚠️ Đã lưu nháp [${name}] vào [${xe}].\nHãy bổ sung tọa độ sau!`);
        } else { alert("❌ Lỗi API: " + result.loi); document.getElementById("single-name").value = name; }
    } catch (e) { alert("⚠️ Không kết nối được máy chủ!"); document.getElementById("single-name").value = name; }
}
...
async function bulkAddCustomers() {
    ...
    try {
        let res = await fetch(`${API_URL}/api/them-hang-loat`, {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ danh_sach_khach: lines, ten_xe: xe })
        });
        let result = await res.json();
...
async function saveCustomer(id) {
    ...
    try {
        let res = await fetch(`${API_URL}/api/cap-nhat-thong-tin/${id}`, {
            method: "PUT", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ ten_khach_hang: newName, du_lieu_goc: newInput, ten_xe: newXe })
        });
        let result = await res.json();
...
// VÁ LỖI 2: XUẤT BÁO CÁO LẤY ĐÚNG NGÀY THEO GIỜ VIỆT NAM (KHÔNG BỊ LÙI NGÀY)
async function exportAndClearData() {
    if (!confirm("BẠN MUỐN CHỐT CA?\nHệ thống sẽ tải file Báo cáo Excel xuống máy bạn, sau đó làm sạch bản đồ để chuẩn bị cho ngày mai. Bạn đồng ý chứ?")) return;
    
    let reportData = customers.map((c, index) => ({
        "STT": index + 1,
        "Tên Xe": c.ten_xe,
        "Mã KH / SĐT": c.ma_khach,
        "Tên Khách Hàng": c.name,
        "Trạng Thái": c.trang_thai,
        "Ghi Chú Tài Xế": c.ghi_chu || "",
        "Dữ liệu gốc": c.rawInput
    }));
    
    if (reportData.length > 0) {
        let wsSheet = XLSX.utils.json_to_sheet(reportData);
        let wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, wsSheet, "BaoCaoGiaoHang");
        
        let d = new Date();
        let dateStr = d.getFullYear() + "-" + String(d.getMonth() + 1).padStart(2, '0') + "-" + String(d.getDate()).padStart(2, '0');
        XLSX.writeFile(wb, `BaoCaoGiaoHang_${dateStr}.xlsx`);
    } else {
        alert("Chưa có dữ liệu giao hàng nào để xuất báo cáo!");
    }
    ...
    try {
        ...
        let result = await res.json();
        if (result.thanh_cong) {
            alert("✅ Đã xuất báo cáo và chốt ca thành công!");
            location.reload(); 
        } 
    } catch (e) { alert("⚠️ Lỗi kết nối mạng khi chốt ca!"); }
}

function toggleMapLabels() { renderCustomerList(); renderMasterDataOnMap(); }

function toggleCustomer(id, isChecked) {
    let index = customers.findIndex(c => c.id === id);
    if (index > -1) { customers[index].selected = isChecked; renderCustomerList(); }
}

function toggleSelectAll(selectAll) {
    let keyword = removeVietnameseTones(document.getElementById("search-input").value.trim());
    customers.forEach(c => {
        if (currentVehicleFilter === "all" || c.ten_xe === currentVehicleFilter) {
            if (!keyword || removeVietnameseTones(c.name).includes(keyword) || c.ma_khach.toLowerCase().includes(keyword)) {
...
function renderCustomerList(editId = null) {
    ...
    let keyword = removeVietnameseTones(document.getElementById("search-input").value.trim());
    let filtered = customers.filter(c => currentVehicleFilter === "all" || c.ten_xe === currentVehicleFilter);
    ...
    filtered.sort((a, b) => {
        let ta = a.thu_tu || 9999;
        let tb = b.thu_tu || 9999;
        return ta - tb;
    });
    ...
    clusters.forEach(cluster => {
        if (cluster.length === 1) { cluster[0].dir = 'right'; }
            ...
            cluster.sort((a, b) => a.lng - b.lng);
            cluster.forEach((p, idx) => { p.dir = dirs[idx % 4]; });
        }
    });
    ...
    filtered.forEach(c => {
        ...
        if (c.id === editId) {
            let vehicleOptions = vehicles.map(v => `<option value="${v}" ${c.ten_xe === v ? 'selected': ''}>${v}</option>`).join('');
            ...
            li.innerHTML = `
                ...
                <div style="font-size: 11px; color: #555; margin-bottom: 2px; margin-top: 5px;">Chuyến xe (Điều hướng):</div>
                <select id="edit-xe-${c.id}" style="width:100%; padding:6px; margin-bottom:8px; border:1px solid #ccc; border-radius:4px;">${vehicleOptions}</select>
            ...
            let badge = `<span class="route-badge">${c.ten_xe}</span>`;
    ...
    if (editId === null && boundsPoints.length > 1) {
...
async function calculateOptimizedRoute() {
    ...
    for (let i = 0; i < routesToProcess.length; i++) {
        ...
        try {
            ...
            if(updateOrderData.length > 0) {
                fetch(`${API_URL}/api/cap-nhat-thu-tu`, {
                    method: "PUT", headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(updateOrderData)
                }).catch(e => console.log("Lỗi đồng bộ thứ tự"));
            }
...
function renderDanhBaList(editDbId = null) {
    ...
    if(keyword) filtered = masterData.filter(d => d.ma_khach.toLowerCase().includes(keyword) || removeVietnameseTones(d.ten_khach).includes(keyword));
    ...
    filtered.forEach(d => {
        ...
        let row = document.createElement("div"); row.className = "db-row"; row.innerHTML = html; container.appendChild(row);
    });
}
...
async function saveDanhBa(id) {
    ...
    try {
        let res = await fetch(`${API_URL}/api/danh-ba/${id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ma_khach: ma, ten_khach: name, du_lieu_goc: input }) });
        let result = await res.json();
        if(result.thanh_cong) { alert("✅ " + result.thong_bao); fetchDanhBaSilently(); fetchDanhBa(); fetchCustomersFromAPI(); } 
        else { alert("❌ Lỗi cập nhật: " + result.loi); }
        ...
        try {
            ...
            rows.forEach((row, index) => {
                ...
                if (row.length >= 1 && row[0]) {
                    danh_sach.push({ ma_khach: row[0].toString().trim(), ten_khach: (row[1] ? row[1].toString().trim() : ""), du_lieu_goc: (row[2] ? row[2].toString().trim() : "") });
                }
            });
...
async function addSingleDanhBa() {
    let ma = document.getElementById("db-new-ma").value.trim(), ten = document.getElementById("db-new-ten").value.trim(), input = document.getElementById("db-new-coords").value.trim();
    if(!ma) { alert("Vui lòng nhập Mã KH / SĐT!"); return; }
    try {
        let res = await fetch(`${API_URL}/api/nhap-danh-ba-excel`, { method: "POST", headers: { "Content-Type": "application/json"}, body: JSON.stringify({ danh_sach: [{ ma_khach: ma, ten_khach: ten, du_lieu_goc: input }] }) });
        let result = await res.json();
        if (result.thanh_cong) {
